...
(() => {
  const DIFFERENTIAL_INTERVAL = 2000;
  const DIFFERENTIAL_TRANSITION = 650;
  const carousel = document.querySelector("[data-differentials-carousel]");
  const viewport = carousel?.querySelector("[data-differentials-viewport]");
  const track = carousel?.querySelector("[data-differentials-track]");

  if (!carousel || !viewport || !track || carousel.dataset.carouselInitialized === "true") return;

  const slides = [...track.querySelectorAll(".differential-slide")];
  ...
  const renderPosition = () => {
    track.style.transform = `translate3d(${-currentPosition * 100}%, 0, 0)`;
  };
  ...
  const nextSlide = () => {
    ...
    currentPosition += 1;
    renderPosition();
    transitionTimer = window.setTimeout(completeTransition, DIFFERENTIAL_TRANSITION + 100);
  };

  const startAutoplay = () => {
    stopAutoplay();
    if (document.visibilityState === "hidden" || isPointerInside) return;
